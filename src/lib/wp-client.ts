...
const EMPTY_CONTENIDO: ContenidoBundle = {
  about: null,
  contact: null,
  home: null,
  servicios: null,
  ...
  if (!contenidoCache) {
    contenidoCache = wpFetch<WPContenido>('contenido')
      .then((rows) => ({
        ...
        // `servicios` (seed v5): textos de la PÁGINA /servicios/. Si falta —el
        // mu-plugin aún no está desplegado— es null y services-page.ts cae a
        // src/i18n campo a campo, sin romper el build.
        servicios: rows.find((r) => r._obliq_key === 'servicios') ?? null,
