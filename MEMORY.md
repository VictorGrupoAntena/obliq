...
> 🔧 **Cómo verificar staging pese al Basic Auth:** nginx devuelve **401 a todo**, incluidas las rutas de rewrite, así que `curl` desde fuera no sirve sin credenciales (que están fuera del repo). Se ataca **Apache directamente por detrás de nginx** vía SSH: `curl -sk --resolve staging.obliqproductions.com:7081:127.0.0.1 https://staging.obliqproductions.com:7081<ruta>`. El puerto **7081** es el backend HTTPS de Apache; el 7080 (HTTP) devuelve antes el 301 a HTTPS de Plesk, y **hay que usar `--resolve`, no `127.0.0.1` a pelo**, o Apache responde `421 Misdirected Request` por SNI. El `:7081` aparece en el `Location`: es artefacto de atacar el backend, no un fallo de la regla.

**🟡 HALLAZGO COLATERAL (fuera del alcance de este sprint) — `/presupuesto/` y `/en/quote/` son URLs huérfanas.** El crawl alcanza 73 de las 75 del sitemap; las 2 que faltan son exactamente las dos páginas de presupuesto, en ambos idiomas. Motivo: el CTA del carrito se emite como `href="#"` y lo rellena **JavaScript** en runtime (`getQuotePageUrl`, `cart-store.ts:211`). **Ninguna otra página las enlaza estáticamente**, así que un rastreador solo llega a ellas por el sitemap y no reciben ningún link equity interno. No lo causó este sprint (el carrito se construyó así) y no se toca aquí; **candidato para el siguiente**.
